      ...
      <ul className="space-y-0 divide-y divide-border">
        {tribunals.map((t) => (
          <li key={t.id}>
            ...
            >
              <span className="font-serif text-lg leading-snug font-medium text-ink transition-colors group-hover:text-accent">
                {t.caseText}
              </span>
              <span className="flex shrink-0 items-center gap-4">
                {t.verdict && (
                  <span className="text-[0.6875rem] font-semibold tracking-[0.1em] text-ink-muted uppercase">
                    {t.verdict}
                  </span>
                )}
